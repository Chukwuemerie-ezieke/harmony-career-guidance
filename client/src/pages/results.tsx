...
import { INTERESTS, getRecommendations, generateWhyText } from "@/lib/courseData";
import {
  GraduationCap, BookOpen, Building2, Briefcase, Clock,
  Printer, Share2, ArrowLeft, Compass, Star,
  CheckCircle2, Target, Award, Link as LinkIcon, Copy
} from "lucide-react";
...
export default function Results() {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-12 text-center">
          <h2 className="text-xl font-bold text-destructive mb-2">Error Loading Results</h2>
          <p className="text-muted-foreground mb-6">Could not load the assessment results. They may have been deleted.</p>
          <Link href="/">
  ...
  const recommendations: RecommendationResult[] = parsedRecs.map((rec: any) => ({
    ...rec,
    whyText: rec.whyText || generateWhyText(rec, subjects, interests, submission.firstName)
  }));
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="hidden print:block text-xs text-muted-foreground mb-4 border-b pb-2">
        <p>Student: {submission.firstName} | Class: {submission.studentClass}</p>
        <p>School: {(submission as any).schoolName || "N/A"} | Date: {new Date(submission.createdAt).toLocaleDateString()}</p>
      </div>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-6 pb-24 sm:pb-6 space-y-6 print:py-0 print:space-y-4" data-testid="results-page">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
          ...
          <div className="flex gap-2 print:hidden flex-shrink-0">
            ...
            <Button size="sm" variant="secondary" onClick={handleGenerateShareLink} className="gap-1.5" disabled={isSharing}>
              ...
              <div className="print:hidden">
                ...
                <div className="mt-4">
                  <h4 className="text-sm font-semibold flex items-center gap-2 mb-2">
                    <BookOpen className="w-4 h-4 text-primary" /> Core Subjects Focus
                  </h4>
                  <div className="bg-muted/30 rounded-lg p-3 space-y-3 border border-border/50">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Based on your strengths, here are the subjects you need for this path. Focus on strengthening subjects you haven't mastered yet.
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {rec.jambSubjects.map((s: string) => {
                        ...
                        const isStrong = subjects.some((sub: string) => sub.toLowerCase().includes(s.toLowerCase()) || s.toLowerCase().includes(sub.toLowerCase()));
                        return (
                          <div key={s} className="flex items-start gap-2 bg-background p-2 rounded border">
                            {isStrong ? (
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {isStrong ? "One of your strong subjects" : "Needs focus & improvement"}
                              </p>
                            </div>
                          </div>
                        );
        ...
        <div className="sm:hidden fixed bottom-0 left-0 right-0 p-4 bg-background border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50 flex justify-between gap-3 print:hidden">
          ...
          <Button variant="secondary" onClick={handleGenerateShareLink} className="gap-1.5 flex-1" disabled={isSharing}>
      ...
      <Dialog open={!!shareLink} onOpenChange={(open) => !open && setShareLink("")}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Shareable Summary Link</DialogTitle>
            <DialogDescription>
              This link provides read-only access to a minimal summary of your results. It will expire in 30 days. No contact details or raw answers are shared.
            </DialogDescription>
          </DialogHeader>
